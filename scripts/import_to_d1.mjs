 ...
 *
 * import.csv columns: token_hash,household_label,phone_e164,source_list,guests
 *   guests: pipe-delimited entries such as "Alex Guest:welcome,ceremony"
 *
 * Usage:
 *   # Print SQL to stdout
...
async function readImportCsv() {
  ...
  for await (const line of rl) {
    ...
    const guests = (guests_raw ?? '').split('|').map(s => s.trim()).filter(Boolean).map(entry => {
      ...
      if (colonIdx === -1) return { name: entry, events: 'welcome,ceremony,farewell' };
      return { name: entry.slice(0, colonIdx).trim(), events: entry.slice(colonIdx + 1).trim() || 'welcome,ceremony,farewell' };
    });
...
function generateSql(rows) {
  ...
  for (const row of rows) {
    ...
    for (const guest of row.guests) {
      const invitedBy = ['A', 'B'].includes(row.source_list)
        ? guest.events.split(',').map(() => row.source_list).join(',')
        : '';
      lines.push(
        `INSERT OR IGNORE INTO guests (invite_id, full_name, events, invited_by)` +
        ` VALUES (` +
