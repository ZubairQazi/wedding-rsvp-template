...
const EVENT_META = {
  ...
  ceremony:  { label: 'Ceremony',       day: 'Saturday, September 19', time: '4:00 PM',  address: 'Venue address shared with guests', image: '../assets/event-placeholder.svg' },
  farewell:  { label: 'Farewell Brunch', day: 'Sunday, September 20', time: '11:00 AM', address: 'Venue address shared with guests', image: '../assets/event-placeholder.svg' },
};
const EVENT_ORDER = ['welcome', 'ceremony', 'farewell'];

// ── Demo mock data ──────────────────────────────────────────────────
const DEMO_DATA = {
  household_label: 'Example Household',
  guests: [
    { id: 1, full_name: 'Alex Guest',  events: ['welcome', 'ceremony', 'farewell'], invited_by: { welcome: 'A', ceremony: 'B', farewell: 'A' } },
    { id: 2, full_name: 'Jordan Guest', events: ['ceremony', 'farewell'], invited_by: { ceremony: 'B', farewell: 'A' } },
  ],
  rsvps: [],
};
...
// ── Guest card builder ───────────────────────────────────────────────
// guest.events is a subset of EVENT_ORDER.
...
function buildGuestCard(guest, rsvpMap) {
  const eventsHtml = guest.events.map(evt => {
    ...
    const ib = guest.invited_by?.[evt];
    const ibLabel = ib === 'A' ? 'Invited by <strong class="ib-partner-a">Partner One</strong>'
                  : ib === 'B' ? 'Invited by <strong class="ib-partner-b">Partner Two</strong>' : '';
      ...
      <div class="event-rsvp-row" data-guest-id="${guest.id}" data-event="${evt}">
        <div class="event-row-top">
          <div class="event-chip-col">
            ...
            ${ibLabel ? `<span class="event-invited-by">${ibLabel}</span>` : ''}
          </div>
